...
function asSymbolInformations(values) {
    return values.map(asSymbolInformation);
}
...
export function asSymbolInformation(item: proto.SymbolInformation, uri?: code.Uri): code.SymbolInformation {
    ...
    let result = new code.SymbolInformation(item.name, item.kind - 1, item.location.range, item.location.uri ? item.location.uri : uri);
